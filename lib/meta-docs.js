...
function docFile(file, rules)
{
  ...
  const after  = file.after ?? (doExt ? mdExt : null);
...
/**
 * Run all currently defined build stages
 * @param {object} rules - Build rules;
 * See `mkdirs` and `docFiles` for supported rules.
 */
function runAll(rules)
{
  mkdirs(rules);
  docFiles(rules);
}
...
module.exports =
{
  fsOpts, mdExt, mkdirs, docFile, docFiles, runAll,
}
